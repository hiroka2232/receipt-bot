...
| `gsheets.py` | Google スプレッドシートへ記録（月シート型フォーマット） |
...
月ごとに1シート（年度は **2月始まり**: `２月`〜`１月`）＋集計用の `全体収支` シート。
空のスプレッドシートを渡せば、初回記録時（または `!初期化` コマンド）に13シートを自動生成する。

各月シートは支出・収入を1つの表に並べ、`種別` 列で区別する:

| 日付 | 種別 | 内容 | 金額 | 領収書等 | 備考 |
|---|---|---|---|---|---|
| 2026-07-17 | 支出 | 九大生協（…） | ¥274 | 20260717_九大生協.jpg | AI自動記録（Gemini） |

- 合計は列全体を見る `SUMIF` なので **1か月の行数に上限が無い**
- 日付は `YYYY-MM-DD` の実データを持ち、表示だけ「日」
